  ...
  return (
    <div ref={menuRef} className="w-fit relative md:hidden">
      ...
      <div
        className={
          open
            ? "w-full max-w-[400px] fixed top-[3.5rem] z-10 sm:top-16 right-0 flex justify-end"
            : "hidden"
        }
      >
        <div className="w-full h-screen max-h-[40rem] fly-in opacity-0 translate-x-2 bg-background shadow-md">
          <h1 className="border-b-2 text-xl px-5 pt-3 text-sky-400 font-semibold">
            Doline Systems
          </h1>
              ...
              <div className="w-full" key={index}>
                {link.sub_links ? (
                  <DropDown className="w-fit">
                    <Link href={link.href}>
                      <Trigger
                        className="w-full justify-between px-5 rounded-none"
                        onClick={handleClose}
                      >
                        <span>{link.name}</span>
                    ...
                    <DropContent
                      position="center"
                      className="shadow-md pt-5 w-[300px]"
                    >
