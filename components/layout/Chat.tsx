...
const ChatBoard = () => {
  ...
  return (
    <div
      ref={board}
      className="fixed bottom-5 right-5 sm:right-10 lg:right-20 "
    >
      {open ? (
        <div
          className={`w-[90vw] flex flex-col max-w-80 md:max-w-96 opacity-0 translate-y-2 ${
            open ? "fly-in" : "fly-out"
          } border bg-background rounded-lg shadow-lg overflow-hidden`}
        >
          ...
          <div className="h-full overflow-y-auto flex flex-col">
            <div className="flex-1 flex flex-col gap-3 overflow-y-auto p-5">
              {messages.map((message, index) => (
                <div
                  key={index}
                  className={`w-fit max-w-[90%] opacity-0 text-sky-100 rounded-lg px-5 py-2 ${
                    message.sender === "sys"
                      ? "bg-emerald-700 -translate-x-[.5rem] fly-right delay-300"
                      : "ml-auto bg-sky-700 translate-x-[.5rem] fly-left delay-500"
                  }`}
                >
                  {message.text}
                </div>
